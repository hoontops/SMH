...
if (nexacro.ExcelExportObject) {
	...
	_pExcelExport.set_exportstyle = function (v) {
		if (v != this._exportstyle) {
			this._exportstyle = v;
		}
		return v;
	};
	...
	// 2015.11.18 add argsDsParam arguments
	_pExcelExport.exportData = function (argsParam,argsDsParam) {
		this._argsParam = argsParam;
		this._argsDsParam = argsDsParam;	// 2015.11.18
		var ret = -1;
		if (!this.exporturl) {
			return ret;
		}
	...
	_pItem._makeforDsStyle = function (ds_style, align, background, color, font, line, cell_type, row_merge, col_merge, ishead) {
		var rt;
		var cellStyleinfo = "";

		if(this.parent && this.parent._exportstyle == false){
			if(ishead){
				background = "229,229,229";
				color = "51,51,51";
				//color = "0,0,0";	//jsh 20161012
				if(font.indexOf("bold") == -1) font = "bold"+font;
				line = "empty:empty:000,000,000:000,000,000";
			}
		}

		if (this._applyA) {
			if(cell_type.indexOf("number")>=0) align="right,middle";	//jsh 20161011 number는 오른쪽 정렬
	...
	_pItem._getCellBodyStyle = function (cell, idx, defaultLineColor) {
		...
		_color="0,0,0";	//jsh 20161006
		_color2="0,0,0";	//jsh 20161006
...
//엑셀 다운로드 에러로 인하여 임시 보완코드
			...
			for (var i = 0; i < hLen; i++) {
				...
				if (subL) {
					for (var j = 0; j < subL; j++) {
						...
						style = this._makeforDsStyle(ds_style, align, bg, color, font, linestyle, cell_type,"","",true);
		...
		if (sLen > 0 && this._applySumm) {
			...
			for (var i = 0; i < sLen; i++) {
				...
				//color = this._getHEXtoRGB(this._getCellStyle(cell, -2, false, "color", "normal"));	//20160330 jsh
				//color = "51,51,51"; //20160330 jsh
				color = "0,0,0"; //20161012 jsh
				if (!d_HLColor) {
					defaultHL = hCells[0].parent._find_gridpseudo_obj("cellline", "normal", null, "line");
	...
	_pItem._gridExportContinue = function (exportObj) {
		...
		var export_dsparam = null;  // 2015.11.18
		if (bodycntcell > 0) {
			var kk = 0;
			var selectedrow = 0;
			...
			if (partitionRow >= bodycntrow) {
				...
				var export_dsparam = this.parent._argsDsParam;  // 2015.11.18
			}
			...
			for (var k = this._startRow; k < partitionRow; k++) {
				...
				if(!exportObj._exportstyle){
				   for (var j = 0; j < bodycntcell; j++) {

					   	  var display_text = this._getCellTextBody(grid, k, j);
						  var val = display_text;
						  var delimiter = String.fromCharCode(29);
						  
						  var cell = cells[j];
						  var cell_type = this._getFixedCellType(cell, k);
						  var style_name, backgroundCell, alignCell, fontCell, colorCell, lineCell;
						  
						  alignCell = cell_type=="number" ? "right,middle" : "center,middle";
						  
						  backgroundCell = "255,255,255";
						  colorCell = "51,51,51"; 
						  //colorCell = "0,0,0";	//jsh 20161012 
						  fontCell = ",9,Dotum";
						  lineCell = "empty:empty:000,000,000:000,000,000";
						  row_suppress_count = 0;
						  col_suppress_count = 0;
						  ...
						  ds.setColumn(idx, "Column" + (j + subcnt), nexacro._nvl(val, "") + delimiter + style_name );
				   }
				}
				else{
					for (var j = 0; j < bodycntcell; j++) {
	...
	_pItem._exportCallback = function (svcid, errstatus, message) {
		...
		if (errstatus < 0) {
			...
			/* 2015.12.23 엑셀 다운로드 URL처리 코드 추가 */
			var excelURL_Con = excelURL.split("8443");
			if(excelURL_Con[1] != null ) {
			 excelURL = excelURL_Con[1];
			}
			/* ********* END OF CODE ********** */
	...
	_pItem._gridItemExport = function (exportObj) {
		...
		if (bodycntcell > 0) {
			...
			var partRowCount = this._partitionRow = parseInt(100000 / str_lenth);
			...
			for (var k = 0; k < partRowCount; k++) {
				...
				if(!exportObj._exportstyle){
				   for (var j = 0; j < bodycntcell; j++) {
						  ...
						  backgroundCell = "255,255,255";
						  //colorCell = "255,110,99";	
						  colorCell = "51,51,51";	//2016-03-30 JSH
						  //colorCell = "0,0,0";	//2016-10-12 JSH
						  fontCell = ",9,Dotum";
						  lineCell = "empty:empty:000,000,000:000,000,000";
						  row_suppress_count = 0;
						  col_suppress_count = 0;
