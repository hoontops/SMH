...
@Service
public class PCM03300ServiceImpl extends AbstractNbdfService implements PCM03300Service {
	
    public static final String NAMESPACE 						= "smh.pcm.dao.PCM03300Dao";
    ...
    * @remark		: 상세설명
* @see smh.pcm.service.PCM03300Service#selectOtherInfo(com.nbdf.commons.ui.vo.UiMapDto)
    */
    @Override
    public void selectOtherInfo(UiMapDto dto) throws Exception {
        ...
        putDataSetMap(dto, generalDao.addSelect(NAMESPACE + ".selectEquipmentListForBatchTrackIn", param),		"output3");        
    }
    ...
    */
    ...
    public void selectBatchTrackIn(UiMapDto dto) throws Exception {
        Map<String, Object> param = getParametersMap(dto);        
        putDataSetMap(dto, generalDao.addSelect(NAMESPACE + ".selectBatchTrackIn", param));        
    }
    ...
    */
    ...
    public void saveLotBatchTrackIn(UiMapDto dto) throws Exception {
		...
		String 							workers					= StringUtil.Object2String(dataInfo.get("WORKERS"));
		...
		String 							firstWorker 				= workers.split(",")[0];
		...
		for(String lotId : lotIdArray) {
			...
			if (!LotProcessState.Wait.equals(processState)) {
				// Lot이 작업 시작 가능 상태가 아닙니다. {0}
				throw new BizException("LotStateIsNotAvailableTrackIn", String.format("LotId = %s", lotId));
			}
			...
			// 작업자 저장
			this.SaveLotWorker(lotData, workers);
			...
			wipService.saveLotInfoByPreviousStep(dto, lotData, TransitionId.TrackIn, firstWorker, now, equipmentId); 
			
			// Track In
			lotData = wipService.trackInLot(dto, lotData, equipmentId, firstWorker, now);			
			lotData = UpdateLotTrackIn(lotData, firstWorker, now); 
			
			// 실적 기록
			wipService.saveLotWorkResult(dto, lotData, TransitType.TrackInLot, firstWorker, now, "", "");
		}   
    }
	...
	***************************************************************************************/
	...
	/**
		...
		* Track In Time 업데이트
		***************************************************************************************/
		private Map<String, Object> UpdateLotTrackIn(Map<String, Object> ld, String userId, Date time) throws Exception
		{
			ld.put("TRACKINTIME",time);
			ld.put("TRACKINUSER",userId);
			ld.put("LASTTXNID",this.getClass().getSimpleName());
			
			generalDao.addUpdate(NAMESPACE + ".updateTrackInTime", ld);
