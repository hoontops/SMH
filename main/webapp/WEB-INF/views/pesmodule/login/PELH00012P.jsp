	...
	<script type="text/javascript">
	   ...
	   function fn_confirm(){
		   ...
		   if(oldPwVal == newPwVal1){
			   alert("변경하고자 하는 비밀번호가 이전 비밀번호와 같습니다. 다른 비밀번호를 입력해 주세요.");
			   return;
		   }
		   ...
		   if(!isValidPassword(newPwVal1)){
			   alert("비밀번호는 영어+숫자+특수문자 조합으로 8자리 이상 15자리 이어야 합니다.");
			   $("#newPassword1").val("");
			   $("#newPassword2").val("");
			   $("#newPassword1").focus();
			   return;
		   ...
		   for(var i=0; i < oldPwVal.length;i++){
			   oldPwVal = oldPwVal.replace("+", "%2B");
			   oldPwVal = oldPwVal.replace("&", "%26");
		   }
		   //PW 변경 서버 호출
		   var data = "oldPassword="+oldPwVal + "&newPassword=" + newPwVal1 + "&loginId=" +  $("#loginId").val() + "&userId=" + $("#userId").val();

          jQuery.ajax({
                    url:"/smh/updateChangeUserPw.scr"
                    ,type:"post"
                    ,data:data
                    ,dataType:"json"
...
<body oncontextmenu="return false">
    <!-- popup size : 520 x 330 -->
    <div class="popWrap">
        <h1><span>비밀번호 변경</span></h1>
        <div class="popcont">
             <input type="hidden" id="userId" value="${userId }" />
             <p class="txt_guide pl15">비밀번호는 <span class="c_point">영문+숫자+특수문자 조합으로 8자리 이상 15자리 이하</span>이어야 합니다.</p>
             <table class="write_t01">
                 ...
                 <tbody>
                     <tr>
                         <th scope="row"><label for="">로그인ID</label></th>
                         <td><input type="text" name="loginId" id="loginId" value="${loginId}"></td>
                     </tr>
                     <tr>
                         <th scope="row"><label for="">이전 비밀번호</label></th>
                     ...
                     <tr>
                         <th scope="row"><label for="">새 비밀번호 확인</label></th>
