...
//날짜(yyyy-mm-dd)만 입력 허용(숫자 + '-') || (event.keyCode != 45)
function onlyDate() {
...
function checkEmail(email, required) {
	var result = false;
	var trimedEmail = validationTrim(email);
...
////////////////////////////////////////////////////////////////////////////////////
// 개월, 일 계산 : getDifMonths(start, end) - start, end format: yyyymmdd
...
{
    var dateStart = new Date(start.substring(0, 4), start.substring(4, 6) - 1, start.substring(6, 8)-1);
...
//연속된 3자리 숫자
function is3NumContinue(password){
   var num = "0123456789";
   for(i=0;i < password.length;i++){
    	if(password.length - i < 3) return true;
        if(num.indexOf(password.substring(i,i+3)) >= 0 ){
        	alert("연속된 숫자 3자리가 포함되어 있습니다.");
		    return false;
        }    
	}
    return true;
...
//3자리 동일문자
function isSame3Str(password){
   for(i=0;i < password.length;i++){
    	if(password.length - i < 3) return true;
    	var str = password.substring(i,i+1)+password.substring(i,i+1)+password.substring(i,i+1);
        if(password.indexOf(str) >= 0 ){
        	alert("3자리의 동일한 문자가 포함되어 있습니다.");
		    return false;
        }    
	}
    return true;
...
// 입력되는 글자수 체크 함수(한글 3byte, 기타1byte로 계산) - PEMI00040M.jsp 회수 메세지 계산에 쓰임
String.prototype.calcBytes = function(){
	var msg = this;
	var cnt = 0;

	for( var i = 0; i< msg.length; i++){
		cnt += (msg.charCodeAt(i) > 128) ? 3: 1;
	}
	return cnt;
};
