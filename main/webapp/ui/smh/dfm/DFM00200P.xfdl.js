(function()
{
    return function()
    {
        ...
        // User Script
        this.addIncludeScript("DFM00200P.xfdl","lib::lib_tom.xjs");
        this.addIncludeScript("DFM00200P.xfdl","lib::lib_mtm.xjs");
        this.registerScript("DFM00200P.xfdl", function() {
        /***************************************************************************************
         * 시스템명 	: IFC MES시스템
        ...
        this.fn_init = function (obj, e)
        {
        	...
        	this.fv_lotid	= this.parent.arg_LotId;
        ...
        this.fn_setImget = function (nRate)
        {
        	...
        	*/
        	...
        	//var nH 	= (parseInt(this.fv_h) * (parseInt(nRate)/100));		// 높이 (여백필요)	-> 이미지 뷰어 사이즈
        	//var nW 	= (parseInt(this.fv_w) * (parseInt(nRate)/100));		// 넓이 (여백필요)	-> 이미지 뷰어 사이즈

        	var nH 	= ((parseFloat(this.fv_h)*1.02) * (parseFloat(nRate)/100));		// 높이 (여백필요)	-> 이미지 뷰어 사이즈(21.06.10)
        	var nW 	= ((parseFloat(this.fv_w)*1.02) * (parseFloat(nRate)/100));		// 넓이 (여백필요)	-> 이미지 뷰어 사이즈(21.06.10)

        	var nR	= (parseInt(nRate)/100);

        	// [20% 증가시킨 사이즈 - 차이]
        	var nH2 = (((parseFloat(this.fv_h)* 1.02) - this.fv_h) * 0.5)* nR;		// [21.06.10]
        	var nW2 = (((parseFloat(this.fv_w)* 1.02) - this.fv_w) * 0.5)* nR;		// [21.06.10]
        	...
        	// 내부 div 설정(겉)   [21.06.10] - 여백을 위해서 추가
        	//var objDivIn = new Div("div_in_panel_img", 0, 0, nW, nH);
        	var objDivIn_M = new Div("div_in_panel_img_M", 0, 0, nW+10, nH+10);		// [21.06.10]
        	...
        	// 내부 div 설정
        	//var objDivIn = new Div("div_in_panel_img", 0, 0, nW, nH);
        	var objDivIn = new Div("div_in_panel_img", 5, 5, nW, nH);		// [21.06.10]
        	objDivIn_M.addChild("div_in_panel_img" ,objDivIn);
        	objDiv.addChild("div_in_panel_img" ,objDivIn);
        	...
        	{
        		this.ds_imgPoint.filter("LAYERID =='"+this.fv_layer+"' && LOTID =='"+this.fv_lotid+"'");
        	}
        	else
        	{
        		this.ds_imgPoint.filter("LAYERID =='"+this.fv_layer+"' && PANELID =='"+this.fv_panel+"' && LOTID =='"+this.fv_lotid+"'");
        	}

        	trace(" === [필터 조건] === :  "+"LAYERID =='"+this.fv_layer+"' && PANELID =='"+this.fv_panel+"' && LOTID =='"+this.fv_lotid+"'" );
        	...
        	if(nPointCnt != 0)
        	{
        		...
        		for(var j=0; j < nPointCnt; j++)
        		{
        			...
        			objStaPoint.init( "sta_panel_point_img_"+j
        							//, (nX*nR)
        							, ((nX*nR) + nW2)					// [21.06.10] 2% 증가된값에 반정도 시작 정보만큰 더함 (0 -> 5)
        							//,  (nH - (nY*nR))
        							, ((nY * nR) + nH2)					// [21.06.10] (기준점 왼쪽하단 -> 왼쪽상단으로 변경)
        							, nPoint, nPoint
        							);			// left, top  (1/4 -> 4씩, 1/2 -> 2씩 같음)
        ...
        this.loadIncludeScript("DFM00200P.xfdl");
