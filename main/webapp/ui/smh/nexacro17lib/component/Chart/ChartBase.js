	...
	_pChartAxisControl._setupTickGeneration = function (width, height) {
		...
		if (!this._tickGenerator) {
			this._tickGenerator = function (axis, istotal) {
                ...
                //2021.07.27 tickinterval에 따라 min, max 값이 보이지 않는 경우가 있어 tickinterval 상관없이 보이도록 설정
                if (v != this._min ){ // tickinterval 로 구한값과 실제 max 값이 같을 경우 제외하지 않으면 겹쳐보이는 현상 방지
                  ticks.push(this._min);
                }

                if (v != max ){ // tickinterval 로 구한값과 실제 max 값이 같을 경우 제외하지 않으면 겹쳐보이는 현상 방지
                  ticks.push(max);
                }
                //2021.07.27 END
				return ticks;
