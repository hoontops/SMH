...
//	arg_year = yymm.substring(0, 4);
 ...
 for(int i=0; i<jsonArray.size(); i++){
	...
	//	System.out.println("strPlanMonth : " + strPlanMonth);
 ...
 sb.append("<dataset seriesName='주택취사난방(2017년)' color='8BBA00'>");
 ...
 for(int i=0; i<jsonArray.size(); i++){
		...
		sb.append("<set value='" + strPlanMonth + "' toolText = '주택취사난방(2017년) {br}" + strPlanMonth + " ㎥' link='javascript:Search2(&quot;" + strYM + "&quot;)' />");
	} 

sb.append("</dataset>");

sb.append("<dataset seriesName='주택취사난방(2018년)' >");

for(int i=0; i<jsonArray.size(); i++){
		JSONObject jsonobj = (JSONObject)jsonArray.get(i);
		
		String strYM = (String)jsonobj.get("arg1");
		String strResultMonth  = (String)jsonobj.get("arg5");
		
		sb.append("<set value='" + strResultMonth  + "' toolText = '주택취사난방(2018년) {br}" + strResultMonth  + " ㎥' link='javascript:Search2(&quot;" + strYM + "&quot;)' />");
	} 

sb.append("</dataset>");
 
sb.append("<dataset seriesName='주택취사(2017년)' parentYAxis='S' renderAs='Line' color='0358B5' >");

for(int i=0; i<jsonArray.size(); i++){
		JSONObject jsonobj = (JSONObject)jsonArray.get(i);
		
		String strYM = (String)jsonobj.get("arg1");
		String strPlanYear = (String)jsonobj.get("arg2");
	//	if(Integer.parseInt(strPlanYear) != 0){
			sb.append("<set value='" + strPlanYear + "' toolText = '주택튀사(2017년) {br}" + strPlanYear + " ㎥' link='javascript:Search2(&quot;" + strYM + "&quot;)' />");	
//		}
	}
...
sb.append("<dataset seriesName='주택취사(2018년)' parentYAxis='S' renderAs='Line'  >");

for(int i=0; i<jsonArray.size(); i++){
		JSONObject jsonobj = (JSONObject)jsonArray.get(i);
		
		String strYM = (String)jsonobj.get("arg1");
		String strPlanYear = (String)jsonobj.get("arg3");
	//	if(Integer.parseInt(strPlanYear) != 0){
			sb.append("<set value='" + strPlanYear + "' toolText = '주택튀사(2018년) {br}" + strPlanYear + " ㎥' link='javascript:Search2(&quot;" + strYM + "&quot;)' />");	
//		}
	}
...
<script src="./js/FusionCharts.js"></script>
 ...
 <script type="text/javascript">
	 <!--
	var chart1 = new FusionCharts("MSColumn3DLineDY.swf","Business", "950", "330");
